...
contract GasStationFactory is EIP712, AccessControl {
    ...
    // EIP-712 type hashes
    bytes32 private constant DEPLOY_MACHINE_TYPEHASH = keccak256("DeployMachineSmartAccount(address eoa,uint256 nonce)");
    ...
    event MetaTransactionExecuted(address indexed user, address indexed relayer, address target, bytes functionCall);
    event MachineSmartAccountDeployed(address indexed deployedAddress);
    event GasStationBalanceTransferred(
        address indexed oldGasStation, address indexed newGasStation, uint256 amount, uint256 nonce
    );
    event TransactionExecuted(address indexed target, bytes data, uint256 nonce, address indexed executor);

    constructor(address admin, address gasStation) EIP712("GasStationFactory", "1") {
     ...
     */
    function deployMachineSmartAccount(address eoa, uint256 nonce, bytes calldata signature)
        external
        onlyRole(GAS_STATION_ROLE)
        returns (address)
    {
        if (eoa == address(0)) revert Errors.ZeroAddress();

        bytes32 structHash = keccak256(abi.encode(DEPLOY_MACHINE_TYPEHASH, eoa, nonce));
        ...
        MachineSmartAccount newMachineSmartAccount = new MachineSmartAccount(eoa, address(this));
     ...
     * @param signature The signature verifying the owner's tx approval.
     */
    function transferGasStationBalance(address newGasStationAddress, uint256 nonce, bytes calldata signature)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        ...
        bytes32 structHash = keccak256(abi.encode(TRANSFER_BALANCE_TYPEHASH, newGasStationAddress, nonce));
        ...
        uint256 gasStationBalance = IERC20(FUNDING_TOKEN).balanceOf(address(this));
        IERC20(FUNDING_TOKEN).safeTransfer(newGasStationAddress, gasStationBalance);

        emit GasStationBalanceTransferred(address(this), newGasStationAddress, gasStationBalance, nonce);
    }
    ...
    function executeMachineTransaction(
        ...
        bytes32 messageHash = keccak256(abi.encodePacked(address(this), eoa, target, data, nonce));
        ...
        if (FUNDING_TOKEN != address(0) && (target == PEAQ_DID || target == PEAQ_RBAC || target == PEAQ_STORAGE)) {
            // Fetch machine's balance
            uint256 machineBalance = IERC20(FUNDING_TOKEN).balanceOf(machineAddress);
            ...
            if (machineBalance <= MIN_BALANCE) {
                ...
                IERC20(FUNDING_TOKEN).safeTransfer(machineAddress, FUNDING_AMOUNT);
            }
        }

        // Forward the call to the machine account to execute the target tx
        MachineSmartAccount(machineAddress).execute(target, data, nonce, eoaSignature);
    }
